...
import { ASSET_STATUS_LIST, ASSET_CATEGORY_LIST } from '../utils/constants';
...
const AssetsPage = () => {
  ...
  const { data, loading, refetch } = useFetch(
    () => assetApi.getAll({ ...filters, page, limit: 12 }),
    [filters.status, filters.category, filters.search, page]
  );

  const canCreate = [ROLES.SYSTEM_ADMIN, ROLES.ASSET_MANAGER].includes(user?.role);
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <select value={filters.status} onChange={(e) => handleFilterChange('status', e.target.value)} className="input-field w-auto">
          ...
          {ASSET_STATUS_LIST.map((s) => <option key={s} value={s}>{s}</option>)}
        </select>
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {data?.map((a) => <AssetCard key={a._id} asset={a} />)}
          </div>
          {data?.length === 0 && <p className="text-center text-sm text-gray-500 py-10">No assets found</p>}
        </>
      )}
      ...
      <Modal isOpen={createModalOpen} onClose={() => setCreateModalOpen(false)} title="New Asset" size="lg">
        <AssetForm onSubmit={handleCreate} submitting={submitting} />
      </Modal>
    </div>
  );
};
